...
8. **Action Routing**: The system inserts transaction audit records, creates security alerts, and returns the risk level, score, and recommended action:
   * `ALLOW`
   * `MONITOR`
...
* **Optimized Estimated Loss**: ₹3,43,673.60
* **Estimated Loss Reduction**: ₹20,42,435.20 / 85.6%
...
* **Scenario B — Suspicious Transaction**: High value, new device/IP, high velocity, previous chargebacks.
  * *Expected Output*: `MEDIUM RISK` / `STEP_UP_VERIFICATION`
* **Scenario C — Coordinated Abuse**: Uses shared fingerprints and IPs.
  * *Expected Output*: `MEDIUM RISK` / `POTENTIAL_ABUSE_RING` detected / `MANUAL_REVIEW`
  * *Risk Logic*: The abuse-ring signal escalates the recommended action to `MANUAL_REVIEW` while preserving the independently calculated numerical risk score (which remains `MEDIUM RISK` based on standard metrics, without claiming it must produce `HIGH` risk).
...
## 12. Testing
Verified via **31 automated tests** targeting key logic components:
...
npm test
```
* **Test Status**: `31 passed`, `0 failed`, `0 skipped`.
